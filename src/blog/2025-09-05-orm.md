---
title: "Exploring AiiDA's ORM architecture"
date: 2025-09-05
category: Blog
tags: [architecture]
...
Above the database layer sits the
...
class, which bridges the abstract `BackendNode` interface (discussed below) with the concrete SQLAlchemy models:
...
class defines the interface contract that all database backend implementations must follow:
