---
title: "Simplifications to the installation of AiiDA since v2.0"
date: 2024-09-20
category: Blog
tags: [usability]
...
[^1]:
    ...
    play around with the tool. The absence of the broker and daemon does not allow _submitting_ of jobs, but only
    _running_ them in a blocking manner (remote execution, e.g. on HPC resources, is still possible, though), and SQLite
    can handle concurrent writes to the database worse than PSQL.

[^2]: Installation should be fairly simple, e.g. via `sudo apt install rabbitmq-server` on Ubuntu.
