      ...
      {visibleEntries.length === 0 ? (
        ...
        <div className="reg-list">
          {visibleEntries.map(([key, p]) => (
            <a key={key} className="reg-card" href={`${base}/plugin-registry/${encodeURIComponent(key)}/`}>
              ...
              <div className="reg-card-badges">
                <span className="reg-badge" title={statusTooltip(p.development_status, statusDict)}>
                  <span className={`reg-badge-left ${statusColorClass(p.development_status)}`}>
                    status
                  </span>
                  <span className="reg-badge-right">{p.development_status}</span>
                </span>
                {p.aiida_version && (
                  <span className="reg-badge">
                    <span className="reg-badge-left blue">aiida-core</span>
                    <span className="reg-badge-right">{p.aiida_version}</span>
                  </span>
                )}
