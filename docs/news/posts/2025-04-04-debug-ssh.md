...
We are well aware of the issue, so to solve it, in the upcoming `AsyncSshTransport` plugin, that is intended to be part of the upcoming 2.7 release of AiiDA, the same setup as `~/.ssh/config` file is used to configure the computer's SSH transport.
It can be selected through the `core.ssh_async` `transport` plugin endpoint during `verdi computer setup`, e.g., via the following configuration YAML file:

```yaml
append_text: ''
default_memory_per_machine: 100
description: ''
hostname: daint.alps.cscs.ch
label: alps-async
...
As you can see, it's as simple as running the correct OS `ssh` command during configuration, which is then picked up by AiiDA.
...
We could succesfully avoid issue number 2 presented in this blog post using he `AsyncSshTransport` plugin.

Finally, for further reading, you can find the original Discourse post that lead to this blog story
...
[^4]: Executed on the OS via a Python `os.system` call. The actual constructed command looks something like the following: `ssh -t daint.alps.cscs.ch -l <my-user> -p 22 -i /home/iam/.ssh/cscs-key -o ProxyJump=ela.cscs.ch` (just with special character escapes, and additional bash logic to evaluate if the directory still exists).
