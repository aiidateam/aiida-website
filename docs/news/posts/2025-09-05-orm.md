...
1. **JSON(B) for Flexibility**: The use of PostgreSQL's `JSONB` type (`JSON` in SQLite) for `attributes`, `extras`, and `repository_metadata` provides schema flexibility while maintaining query performance.
...
The `ModelWrapper` is a crucial component that sits between AiiDA's backend entities and raw SQLAlchemy models.
...
The magic happens in `backend.nodes.create()` - this method returns an SqlaNode instance as shown above (automatically selecting the correct backend).
...
This namespace pattern was introduced because having all properties and methods directly on the `Node` class became overwhelming.
