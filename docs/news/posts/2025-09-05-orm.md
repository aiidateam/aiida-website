...
This abstract base class (ABC) ensures that regardless of which ORM implementation is used, the same interface is available to higher-level code.
...
The abstract class serves as a _contract_ —any backend implementation must provide all these methods and properties.

!!! note
    In practice, the `BackendNode` abstract base class is actually only used as the parent class for `SqlaNode`, making this layer of abstraction appear redundant.
...
The creation magic happens in `backend.nodes.create()` which returns an `SqlaNode` instance[^3]
...
and the use of `@cached_property` ensures that these namespace objects are created only once per node instance[^4].
